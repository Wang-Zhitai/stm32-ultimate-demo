...
/**
 * @brief GT911的I2C总线初始化函数，已经在main里面初始化过了
 *
 * @return int32_t
 */
...
{
  HAL_StatusTypeDef status = HAL_I2C_Mem_Write(&hi2c1, DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length, 1000);
  return (status == HAL_OK) ? GT911_OK : GT911_ERROR;
}
...
{
  HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c1, DevAddr, Reg,
...
void BSP_GT911_Init(void)
{
  ...
  GT911_IO_t gt911_io = {
      .Init = I2C_Init,
      .DeInit = I2C_DeInit,
      .Address = 0xBA, // 根据GT911的硬件设置设备地址
      .WriteReg = I2C_WriteReg,
      .ReadReg = I2C_ReadReg,
      .GetTick = GetTick};

  /* 2. 注册IO总线 */
  ret = GT911_RegisterBusIO(&gt911_obj, &gt911_io);
  if (ret != GT911_OK)
  {
    return;
  }
  ...
  /* 4. 读取芯片ID验证通信 */
  uint32_t chip_id;
  ret = GT911_ReadID(&gt911_obj, &chip_id);
  if (ret != GT911_OK)
  {
    return;
  }
